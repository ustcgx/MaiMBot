...
class WillingManager:
    ...
    def get_willing(self,chat_stream:ChatStream) -> float:
        ...
        stream = chat_stream
        if stream:
            return self.chat_reply_willing.get(stream.stream_id, 0.2)
        return 0
    ...
    async def change_reply_willing_received(self, 
        ...
        current_willing = self.chat_reply_willing.get(chat_id, 0.2)
        ...
        reply_probability = max((current_willing - 0.4) * 2, 0)
    ...
    def change_reply_willing_sent(self, chat_stream:ChatStream):
        ...
        stream = chat_stream
        if stream:
            current_willing = self.chat_reply_willing.get(stream.stream_id, 0.2)
            self.chat_reply_willing[stream.stream_id] = max(0.2, current_willing - 2)
